...
def main():
    ...
    '''folder_square = f'{dirname(dirname(sys.argv[0]))}/results/MASSIM_square'
    folder_crop = f'{dirname(dirname(sys.argv[0]))}/results/MASSIM_crop'
    content_square = sorted(os.listdir(folder_square))
    content_crop = sorted(os.listdir(folder_crop))
    diff_low = []
    diff_cl = []
    for i in range(len(content_crop)):
        rad, low_square, cl_square = tsv_to_arr(folder_square + '/' + content_square[i])
        rad, low_crop, cl_crop = tsv_to_arr(folder_crop + '/' + content_crop[i])
    ...
    avg_square = f'{dirname(dirname(sys.argv[0]))}/results/Average_MASSIM_square.tsv'
    ...
    create_plot(rad, diff_low, diff_cl, 'low', 'clinical', 'MASSIM_square(radius) - MASSIM_crop(radius)', 
                'radius', 'difference', np.arange(rad[0], rad[-1], 10), np.arange(min_y, max_y, 0.001), 
                f'{dirname(dirname(sys.argv[0]))}/images')
